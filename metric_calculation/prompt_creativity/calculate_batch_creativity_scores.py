...
class BatchCreativityCalculator:
    ...
    def __init__(
        ...
        use_llm: bool = False,  # LLM 사용 여부 플래그 추가
        api_key: str = None
    ):
        """
        ...
        # Initialize Evaluator with LLM option
    ...
    async def calculate_prompt_scores(self) -> List[PromptCreativityScore]:
        ...
        """
        logger.info(f"Calculating scores (Mode: {'GPT-4 Judge' if self.evaluator.use_llm else 'Heuristic Rules'})...")
        self.prompt_scores = []
...
async def main():
    ...
    # LLM 사용 옵션 추가
    parser.add_argument("--use-llm", action="store_true", help="Use GPT-4 as judge instead of heuristics")
    ...
    calculator = BatchCreativityCalculator(
        ...
        use_llm=args.use_llm,  # Pass flag
